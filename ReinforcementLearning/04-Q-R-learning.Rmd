...
* Derive L-value for the *optimal action* in state $X$ as per *true transition probability* as $L\left(X, \pi^*, P_X(\pi^*)\right) = r(X, a^*) + \sum_Y P_{XY}(a^*)V^*_Y$; where $V^*$ values are extracted after solving the optimality equations using the true transition probability.
...
* For the current state and the chosen action per a policy $\pi^0$, define the delta value as $\Delta(X, \pi^0) = L\left(X, \pi^*, P_X(\pi^*)\right) - L\left(X, \pi^0, P_X(\pi^0)\right)$.
